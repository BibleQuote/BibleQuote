...
type
  ...
  TAlekPageControlDeleteTab = procedure(sender: TAlekPageControl;
  ...
  published
    ...
    property OnDeleteTab: TAlekPageControlDeleteTab read FOnDeleteTab
      write FOnDeleteTab;
...
procedure Register();
begin
  RegisterComponents('BqComponents', [TAlekPageControl]);
end;
{ TAlekPageControl }
...
begin
  ...
  image_width := 13;
  image_height := 13;
  if (image_width < 3) or (image_height < 3) then
    goto default_processing;
  tabRect.Left := tabRect.Right - image_width - 4;
  Inc(tabRect.Top, 3);
...
var
  ix: integer;
begin
  if not assigned(FOnDblClk) then
  begin
    inherited;
    exit
  end;
...
begin
  ...
  try
    ...
    begin
      ...
      if FCloseImage <> 0 then
      begin
        DrawIconEx(Canvas.Handle, r2.Right - image_width - 4, r2.Top + 3,
          FCloseImage, 13, 13, 0, 0, DI_NORMAL);
        // Canvas.Draw(r2.Right - image_width - 4, r2.Top + 3, FCloseImage);
      ...
      Canvas.TextFlags := 0;
      DrawTextW(Canvas.Handle, pCaption, textlength, textRect,
      ...
      if length(ts.Caption) <> length(_caption) then
        ts.Caption := _caption;
      // SetBkMode(Canvas.Handle, TRANSPARENT);
      // DrawTextW(Canvas.Handle, pCaption, savelength, saveRect, DT_CENTER or
      // DT_VCENTER);
