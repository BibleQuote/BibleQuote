...
function GetWMIObjectsCount(Services:ISWbemServices; wmiClass:string):integer;
var
  //SObject:  ISWbemObject;//AlekID:hint
  ObjSet:   ISWbemObjectSet;
begin                                          
  Result := -1;
...
//var
  //Valor:variant;//AlekId:Hint
begin
  // ini
//  AIsnull := False;//AlekId:Hint
  Result := STR_EMPTY;
...
begin
  ...
  if (AObject <> nil) then begin
    ...
    if Assigned(wmiInParams) then begin
      //j := wmiInParams.Properties_.Get_Count;//AlekId:Hint
    ...
    if Assigned(wmiOutParams) then begin
      //j := wmiOutParams.Properties_.Get_Count;//AlekId:Hint
