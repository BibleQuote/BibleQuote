...
//Cancel Operation
  if self.FSevenzip.FMainCancel then begin
    outStream := nil;
    result := S_FALSE;
    exit;
  end;
...
//rg 23.8.06
          ...
          if (attr.uiVal and (1 shl 4)) <> 0 then begin
            if isUnicode then
...
destructor TMyArchiveExtractCallback.Destroy;
begin
  inherited;
end;
...
begin
  try
    ...
    inA.Close;
    inA.Open(ms, nil, nil);
    Result := InternalGetIndexByFilename(FileToExtract, pOutSize);
  finally
    inA.close;
  end
end;
...
begin
 ...
 //ErikGG End 07.11.06
  ...
  if not copyfilew(PWidechar(sfxmodule), PWideChar(DestFn), True) then begin
...
// Shadow 27.11.2006                                           //FHO 17.01.2007
  ...
  //Set archive options
    ...
    //rg 17.04.06
      ...
      if (FPassword <> '') and (AddEnCryptFilename in FAddoptions) then begin
        Setp[NumOfProps].vt := VT_BSTR;
