...
begin
  ...
  if (dLength >= 2) and (aBuffer [1] = #255) and (aBuffer [2] = #254) then
  begin
    dPos := 3;
    dIsMSL := true;
  end else
  if (dLength >= 2) and (aBuffer [1] = #254) and (aBuffer [2] = #255) then
  begin
    dPos := 3;
    dIsMSL := false;
  end else
  ...
  begin
    ...
    if (dChar = #13) or ((dChar = #10) and not dPrevCR) then
    begin
      rStrings.Add (Copy (dCurString, 1, dCurLength));
      dCurLength := 0;
      dPrevCR := (dChar = #13);
    end else
    if dChar = #10 {and dPrevCR, всегда} then
    begin
      dPrevCR := false;
    end else
    begin
...
begin
  ...
  dChar := aBuffer [rPos];
  if dChar in ['A'..'Z', 'a'..'z', '0'..'9', '_', '-', '+'] then
  begin
    rPos := rPos + 1;
...
begin
  ...
  begin // Хак от традиционных для русских ОС редиректов 1252 -> 1251
...
begin
  ...
  for i := 1 to dLength do
  begin
    if aString [i] <> #0 then
    begin
      if fUnicodeToRussianXLAT [Integer (aString [i])] = #0 then
      begin
        Result := true;
        Exit;
      end;
...
begin
  ...
  begin
    if IsContainsNonRussianChars (aStrings.Strings [i]) then
    begin
      Result := true;
      Exit;
    end;
...
begin
  ...
  if (dBuffer [1] = #255) and (dBuffer [2] = #254) then
  begin
    Result := cEncUnicode;
    Exit;
  end else
  if (dBuffer [1] = #254) and (dBuffer [2] = #255) then
  begin
  ...
  while (dPos <= dBufferLength) and (dBuffer [dPos] <> '"') do
  begin
...
begin
  ...
  if IsContentUnicode (dBuffer) then
  begin
...
begin
  ...
  try
    ...
    if (dEncoding = cEncUnicode) or (dEncoding = cEncUnicode_BigEndian) then
    begin
...
begin
  ...
  try
    ...
    begin
      ...
      if dName = 'DesiredFontCharset' then
      begin
