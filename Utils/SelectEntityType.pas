...
var
  FileName: String;
begin
  FileName := ExtractFileName(aFileEntryPath);
  if (FileName = '') then
    Result := False
  else
    Result := TRegEx.IsMatch(FileName.ToLower(), '^[^\.]*(\.sqlite3)?$');
end;
...
class function TSelectEntityType.IsNativeInfoSource(aFileEntryPath: String): Boolean;
begin
