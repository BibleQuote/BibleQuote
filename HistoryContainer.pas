unit HistoryContainer;

interface

uses LinksParserIntf;
...
type
  ...
  public
    ...
    procedure toFile(path: string);
    procedure fromFile(path: string);
  end;

implementation

uses SysUtils, Windows, JCLWin32, ExceptionFrm;
{ TBQHistoryContainer }
...
constructor TBQIntfHistoryContainer.Create(max: integer = 500;
...
begin
  if mCurrentDepth < mMaxDepth then
    result := mCurrentDepth
  else
    result := mMaxDepth;
end;

procedure TBQIntfHistoryContainer.fromFile(path: string);
var
  fHandle: THandle;
  i, c, l, check: integer;
  w: string;
...
begin
  fHandle := CreateFile(
    PChar(Pointer(path)),
    FILE_READ_DATA or STANDARD_RIGHTS_READ, 0, nil, OPEN_EXISTING, 0, 0);
  ...
  try
    ...
    begin
      ...
      if l <> check then
        raise Exception.Create('Unexpected write history file error');
    end;
  except
    on e: Exception do
      BqShowException(e);
...
begin
  ...
  begin
    newLen := mCurPtr * 2;
    if newLen > mMaxDepth then
      newLen := mMaxDepth;
  end;
  SetLength(mContainer, newLen);
end;

function TBQIntfHistoryContainer.IndexOf(
  const match: TBibleLinkEx;
  const matFlags: TBibleLinkExLikeness;
...
begin
  if mCurrentDepth <= 0 then
    raise ERangeError.Create('TBQHistoryContainer underflow: pop from empty containter');
  ...
  if mCurPtr < 0 then
    mCurPtr := mMaxDepth - mCurPtr;
end;

function TBQIntfHistoryContainer.push(const lnk: TBibleLinkEx): integer;
begin
  inc(mCurPtr);
  if (mCurPtr >= ArrLen()) then
  begin
    if (mCurPtr < mMaxDepth) then
    begin
      IncArr();
    end
    else
      Dec(mCurPtr, mMaxDepth);
  end;
  inc(mCurrentDepth);
...
procedure TBQIntfHistoryContainer.toFile(path: string);
var
  fHandle: THandle;
  i, c, l, check: integer;
  w: WideString;
  pbl: PBibleLinkEx;
begin
  fHandle := CreateFile(PChar(Pointer(path)), FILE_WRITE_DATA or STANDARD_RIGHTS_WRITE, 0, nil, CREATE_ALWAYS, 0, 0);
  try
    if fHandle = INVALID_HANDLE_VALUE then
      raise Exception.Create('Cannot write history file');
